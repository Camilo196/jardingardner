...
export function getMongoUri() {
  ...
  if (!uri) {
    throw new Error(
      'Define MONGO_URI, MONGODB_URI o MONGO_URI_DIRECT en el archivo .env'
    );
  }
...
function normalizeMongoConnectionError(error: unknown, uri: string) {
  const mongoError = error as NodeJS.ErrnoException;

  if (mongoError?.code === 'ECONNREFUSED' && mongoError?.syscall === 'querySrv') {
    return new Error(
      'No se pudo resolver el registro DNS SRV de MongoDB Atlas. ' +
