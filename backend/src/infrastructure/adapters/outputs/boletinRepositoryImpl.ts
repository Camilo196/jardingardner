...
import { BoletinModel, BoletinDocument } from '../outputs/models/BoletinModel.js';
...
export class BoletinRepositoryImpl {
  ...
  async findByCursoYPeriodo(cursoId: string, periodo: string): Promise<Boletin[]> {
  ...
  async create(input: Omit<Boletin, 'id'>): Promise<Boletin> {
  ...
  async update(id: string, input: Partial<Omit<Boletin, 'id'>>): Promise<Boletin | null> {
    const updatedBoletin = await BoletinModel.findByIdAndUpdate(
      id,
      input,
      { new: true }
    ).exec();
  ...
  async findByAsignaturaId(asignaturaId: string): Promise<Boletin[]> {
  ...
  private mapToEntity(doc: BoletinDocument): Boletin {
    ...
    const calificaciones = doc.calificaciones.map((calificacion) => {
      return new Calificacion(
        ...
        doc.cursoId,
        calificacion.nota,
        calificacion.periodo, // 👈 Si es `undefined`, usa la fecha actual
        calificacion.observaciones || ''
      );
    });

    return new Boletin(
      (doc._id as string).toString(), // El id del boletín
