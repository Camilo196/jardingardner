...
function obtenerRutaBanner() {
    const candidatos = [
        (0, path_1.resolve)(process.cwd(), '../frondend/src/app/assets/institution/pdf_image_2.jpg'),
        (0, path_1.resolve)(process.cwd(), 'frondend/src/app/assets/institution/pdf_image_2.jpg'),
        (0, path_1.resolve)(process.cwd(), '../frondend/src/app/assets/institution/pdf_image_1.jpg'),
        (0, path_1.resolve)(process.cwd(), 'frondend/src/app/assets/institution/pdf_image_1.jpg'),
    ];
...
function numeroPeriodo(periodo) {
...
function notaComportamiento(comp) {
    ...
    if (!comp?.nivel)
        return null;
    const m = { Superior: 5, Alto: 4, Basico: 3, Bajo: 2 };
    return m[comp.nivel] ?? null;
}
function labelPeriodo(n) {
    return n === 1 ? 'Primer periodo' : n === 2 ? 'Segundo periodo' : 'Tercer periodo';
}
function itemsArr(items, fallback) {
    return items && items.length ? items : [fallback];
}
// ─── Clase principal ──────────────────────────────────────────────────────────
class PDFService {
    constructor() {
        this.bannerPath = obtenerRutaBanner();
    }
    // ── Crear buffer ────────────────────────────────────────────────────────────
    crearBuffer(render, options = { margin: 0, size: 'A4' }) {
    ...
    dibujarPie(doc, data, label) {
        ...
        doc.fillColor(C.azulClaro).font('Helvetica').fontSize(7)
            .text(`Jardin Infantil Gardner | ${label} | ${textoPeriodo(data)} | Sistema Learnscape`, 0, ph - 13, { width: pw, align: 'center' });
    }
    // ── Tabla de datos del estudiante (amarilla) ────────────────────────────────
    dibujarTablaEstudiante(doc, data, x, y, w, _conPeriodo = false) {
    ...
    // ── Firmas ──────────────────────────────────────────────────────────────────
    dibujarFirmas(doc, ml, cw, y, etiquetas) {
    ...
    renderCartaPreescolar(data) {
        return this.crearBuffer((doc) => {
            ...
            const paginaBase = (esFirst) => {
                ...
                if (esFirst) {
                    ...
                    doc.fillColor(C.tinta).font('Helvetica-Bold').fontSize(12)
                        .text(saludo, sX, y);
                    y += 20;
                    const intro = `Reciban un cordial saludo. Durante este ${textoPeriodo(data).toLowerCase()} hemos acompanado a ` +
            ...
            for (const mat of data.calificaciones) {
                ...
                // Calcular alturas
                const resumenH = Math.max(36, doc.heightOfString(resumenTexto, { width: sW - 20 }) + 18);
                // Objetivo + Actividades en una sola sección blanca
                const objActTexto = `-Objetivo: ${objetivo}\n\nActividades:\n` +
                    actividadesItems.map((a) => `- ${a}`).join('\n');
                const objActH = Math.max(56, doc.heightOfString(objActTexto, { width: sW - 20 }) + 18);
                // Observaciones: label fijo + texto (fondo blanco liso)
                const obsText = obsLines.join('\n');
                const observacionesH = Math.max(30, doc.heightOfString(obsText, { width: sW - 20 }) + 18);
    ...
    renderInformeEvaluativo(data) {
        return this.crearBuffer((doc) => {
            ...
            for (const mat of data.calificaciones) {
                ...
                doc.fillColor(C.blanco).font('Helvetica-Bold').fontSize(9.5)
                    .text(mat.asignaturaNombre, sX + 6, top + (fila1H + fila2H) / 2 - 6, { width: nomW - 12, align: 'center' });
                ...
                const dibujarFila = (etiqueta, items, fondo, fondoTexto, alto) => {
                    const etW = 64;
                    const txW = sW - etW;
                    const txt = items.map((s) => `\u2022 ${s}`).join('\n');
            ...
            const labelProm = pNum === 1 ? 'Promedio general I periodo:' :
            ...
            const valW = resW - lbW;
            const rowH = 28;
            const filaResumen = (label, valor, yR) => {
                doc.lineWidth(1);
    ...
    async generateBoletinPDF(boletin, estudiante, curso, calificaciones) {
        const data = {
            ...
            calificaciones: calificaciones.map((cal) => ({
                ...
                valoracion: cal.nota >= 4.5 ? 'Superior' :
                    cal.nota >= 4.0 ? 'Alto' :
